...
exports.createAssignment = async (data) => {
    ...
    const [result] = await db.promise().query(
        ...
        [data.groupId, data.name, data.content, data.userId, new Date(), new Date(), data.postId]
    )
...
exports.getDetailAssignment = async (data) => {
    const [info] = await db.promise().query(
        "SELECT * FROM assignment WHERE postId = ?", [data.assignmentId]
    )
...
exports.updatePharse = async (data) => {
    for(const element of data.steps){
        let [check] = await db.promise().query(
            "SELECT * FROM phrase WHERE id = ?", [element.id]
        )
        if(check.length == 0){
            await db.promise().query(
                "INSERT INTO phrase (assignmentId, name, no, start, end) VALUES (?, ?, ?, ?, ?)",
                [element.assignmentId, element.name, element.no, element.start, element.end]
            )
        }else{
            await db.promise().query(
...
exports.getTask = async (data) => {
    const [result] = await db.promise().query(
        "SELECT * FROM task WHERE phraseId = ?", [data.stepId]
    )
    return result;
}
...
exports.addTask = async(data) => {
    const [result] = await db.promise().query(
        ...
        [data.pic, data.name, 'waiting', '', data.task_giver, data.stepId, new Date(), new Date(), data.dueto]
    )
    return result;
}
...
exports.getTaskByUser = async (data) => {
    const [result] = await db.promise().query(
        "SELECT * FROM task WHERE pic = ?", [data.pic]
    )
    return result;
}